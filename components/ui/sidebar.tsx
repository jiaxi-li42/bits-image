...
// Port of shadcn-ui's Sidebar primitive, with Radix dependencies removed.
// - Mobile mode reuses the project's base-ui Sheet (components/ui/sheet.tsx).
// - Tooltip / icon-rail / Slot.asChild are intentionally not implemented;
//   `collapsible="icon"` is excluded.
...
function useIsMobile(breakpointPx = 768) {
  ...
  React.useEffect(() => {
    ...
    const onChange = () => setIsMobile(window.innerWidth < breakpointPx);
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Persist desktop state across reloads.
  ...
  const toggleSidebar = React.useCallback(() => {
    // Mobile-only — desktop sidebar is permanent and has no toggle UI.
...
function Sidebar({
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* Spacer that reserves layout width when expanded */}
      <div
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        // `no-scrollbar` (from shadcn/tailwind.css) hides the desktop
        // scrollbar so the sidebar matches mobile's near-invisible overlay.
...
// `SidebarMenuLink` is a sidebar-styled wrapper for content that should render
// as something other than a button (e.g. a Next.js <Link>). Equivalent to
// shadcn's `<SidebarMenuButton asChild>` pattern.
...
function SidebarMenuBadge({
  ...
  return (
    <div
      ...
      className={cn(
        "pointer-events-none absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs tabular-nums text-muted-foreground select-none",
        "top-1.5",
        className,
      )}
      {...props}
